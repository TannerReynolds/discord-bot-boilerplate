exports.run = async (client: any, message: any, args: any, level: any) => { // eslint-disable-line no-unused-vars
  ...
  const [globalCmds, guildCmds] = client.container.slashcmds.partition((c: any) => !c.conf.guildOnly);
  ...
  await client.guilds.cache.get(message.guild.id)?.commands.set(guildCmds.map((c: any) => c.commandData));

  // Then set the global commands like 
  await client.application?.commands.set(globalCmds.map((c: any) => c.commandData)).catch((e: any) => console.log(e));
